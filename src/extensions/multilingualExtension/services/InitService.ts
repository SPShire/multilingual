...
import { sp, Fields, PermissionKind, ClientSidePage, ClientSideWebpart, File } from '@pnp/sp';
...
export default class InitService implements IInitService {
  ...
  public async getAllPages(languages: ILanguage[], currentUrl: string): Promise<IPageProperties[]> {
    ...
    const viewXml = `<View Scope='RecursiveAll'><ViewFields><FieldRef Name='ID'/><FieldRef Name='FileLeafRef'/><FieldRef Name='FileRef'/><FieldRef Name='Title'/><FieldRef Name='LanguageVariant'/>`
      + `<FieldRef Name='LastSynced'/><FieldRef Name='MasterTranslationPage'/><FieldRef Name='LanguageFolder'/><FieldRef Name='RedirectorPage'/><FieldRef Name='_UIVersionString'/><FieldRef Name='Modified'/><FieldRef Name='Editor'/></ViewFields><Query></Query></View>`;
    const viewUrl = `${this._context.pageContext.site.absoluteUrl}/_api/web/Lists/GetByTitle('Site Pages')/RenderListDataAsStream`;
    const body = { "parameters": { "RenderOptions": 0, "ViewXml": viewXml } };
        ...
        if (data.Row.length > 0) {
          data.Row.forEach((page) => {
            if (page.ContentType != "Folder") {
              ...
              //if(rootFolder || (match != null && match.length == 2 && (lodash.find(languages, ['code', match[1]])!=null))) {
              if (rootFolder || (match != null && match.length == 2 && (lodash.find(languages, (lang) => { return (lang.code.toLowerCase() == match[1].toLowerCase()); }) != null))) {
                ...
                if (diff > 0) {
                  //minutes
                  //p.DurationSinceSync = Math.round(((diff % 86400000) % 3600000) / 60000);
                  //seconds
  ...
  public async pageEdit(): Promise<boolean> {
    try {
      ...
      Logger.write(`${err} - ${this.LOG_SOURCE} (pageEdit)`, LogLevel.Error);
      return false;
    }
  }
  ...
  public async manageRedirectorPage(redirectorUrl: string, mapping: IMap): Promise<boolean> {
    ...
    try {
      ...
      if (redirectorPage) {
        ...
        //Update
        let result = await redirectorPage.save();
        return true;
      } else {
