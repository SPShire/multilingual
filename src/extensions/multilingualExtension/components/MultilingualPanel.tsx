...
export class MultilingualPanel extends React.Component<IMultilingualPanelProps, IMultilingualPanelState> {
  ...
  // @autobind
  // private doRetranslate(): void {
  //   if (this.state.workingRetranslate) return;
  //   this.setState({
  //     workingRetranslate: true
  //   }, () => {
  //     this.retranslate();
  //   });
  // }
    ...
    return (
      <div className={styles.multilingualPanel + ((this.props.editMode && this.props.showPanel) ? ' ' + styles.show : '')}>
        ...
        <div className={styles.panelDetail}>
          ...
          <Label className={"ms-fontWeight-semibold"}>This Page's Current Language:</Label>
          <Label>{this.state.currentLanguage.TranslationLanguage.description}
            {(this.state.currentLanguage.Page.Id == this.state.currentLanguage.Page.MasterTranslationPage) &&
              <TooltipHost
                content="Master translation page."
                directionalHint={DirectionalHint.topLeftEdge}
                calloutProps={{ gapSpace: 0 }}>
                <i className={styles.iconPanel + " ms-fontColor-redDark ms-Icon ms-Icon--StarburstSolid"} aria-hidden="true"></i>
              </TooltipHost>
            }

            {(this.state.currentLanguage.Page.Version >= 1.0) &&
              <TooltipHost
                content="Translation has been published."
                directionalHint={DirectionalHint.topLeftEdge}
                calloutProps={{ gapSpace: 0 }}>
                <i className={styles.iconPanel + " ms-Icon ms-Icon--WebPublish"} aria-hidden="true"></i>
              </TooltipHost>
            }
          </Label>
          ...
          {this.state.otherLanguages && this.state.otherLanguages.map((t: ITranslation) => {
            return (
              <div key={t.TranslationLanguage.code} className={styles.translationToggle}>
                ...
                />
                <Label className={styles.translationLabel + " ms-fontWeight-semibold"}>{t.TranslationLanguage.description}</Label>
                {t.Page && (t.Page.Id == t.Page.MasterTranslationPage) &&
                  <TooltipHost
                    content="Master translation page."
                    directionalHint={DirectionalHint.topLeftEdge}
                    calloutProps={{ gapSpace: 0 }}>
                ...
                {t.Page && (t.Page.Version >= 1.0) && (t.Page.DurationSinceSync > 30) &&
                  <TooltipHost
                    content="Version has been modified since last automatic translation."
                    directionalHint={DirectionalHint.topLeftEdge}
                    calloutProps={{ gapSpace: 0 }}>
                    <i className={styles.iconPanel + " ms-fontColor-yellow ms-Icon ms-Icon--BranchFork"} aria-hidden="true"></i>
                  </TooltipHost>
                }
              </div>
            );
          ...
          {this.props.variantPages.length > 0 &&
            ...
            //   {this.state.workingRetranslate &&
            //     <Spinner className={styles.spinner} size={SpinnerSize.large} />
            //   }
            //   <DefaultButton
            //     text="Re-Translate"
            //     primary={true}
            //     onClick={this.doRetranslate}
            //     disabled={this.state.pageNotMaster || this.state.hasChanged}
            //   />
            // </div>
            <div>
